// src/soc/webservice/upload/WsUploadArquivo.ts
import { UploadSocged } from 'src/azure/types/azure.types';

const WSSecurity = require('wssecurity-soap');

export async function WsUploadArquivo(payload: UploadSocged): Promise<void> {
  ...
  const CODIGO_SOCGED_EXAME = '16';
  const CLASSIFICACAO_GED = 'RESULTADO_EXAME';
  ...
  // Parte 2: Anexo binário (PDF) - agora sabemos que payload.arquivo não é null
  parts.push(
    Buffer.from(
      `--${boundary}\r\n` +
  ...
  parts.push(payload.arquivo); // TypeScript agora sabe que não é null
