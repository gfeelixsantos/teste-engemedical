...
const StatButton: React.FC<{
  ...
  <button
    aria-label={label}
    className="flex gap-2 items-center justify-center p-2 rounded-lg transition-all duration-200 hover:bg-gray-50 focus:outline-none focus:ring-1 focus:ring-[#28B1CF] min-w-[50px] cursor-pointer"
    onClick={onClick}
    type="button"
    {...rest}
  >
      ...
      {isOpen && (
        <>
          <div className="fixed inset-0 bg-black/45 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-xl w-full h-full max-h-screen overflow-hidden flex flex-col max-w-6xl border border-[#28B1CF]/15">
              {/* Cabeçalho do modal */}
              <div className="p-4 border-b border-[#28B1CF]/15 flex justify-between items-center bg-[#F2F9FC]">
                <div>
                  <h2 className="text-xl font-bold text-[#006782]">
                ...
                <div className="flex items-center gap-2">
                  <button
                    aria-label="Fechar"
                    className="w-7 h-7 flex items-center justify-center text-gray-500 hover:text-gray-700 rounded-lg hover:bg-gray-100 transition-colors focus:outline-none focus:ring-1 focus:ring-[#28B1CF]"
                    onClick={onClose}
                  >
                    <X size={16} />
                  </button>
...
const RECEPCAO_CARDS: CardConfig[] = [
  ...
  { key: "total", label: "Totais", tooltip: "Total de senhas", icon: <Users size={12} />, color: "bg-gradient-to-r from-[#28B1CF] to-[#a6ce39] text-white" },
];
...
const ATENDIMENTO_CARDS: CardConfig[] = [
  ...
  { key: "total", label: "Total", tooltip: "Total de atendidos (aguardando + finalizados)", icon: <Users size={12} />, color: "bg-gradient-to-r from-[#28B1CF] to-[#a6ce39] text-white" },
];
    ...
    <>
      ...
      <div className="lg:hidden">
        <button
          ...
          className="p-2 bg-white border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors focus:outline-none focus:ring-1 focus:ring-[#28B1CF]"
          onClick={() => onSetStatsModalOpen(true)}
        >
          <BarChart3 className="text-[#28B1CF]" size={16} />
        </button>
      </div>
    </>
  );
