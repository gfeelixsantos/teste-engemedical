...
export async function GET(_req: Request, { params }: { params: Promise<{ id: string }> }): Promise<NextResponse> {
  try {
    ...
    const response = await fetch(`${NEST_URL}riscos-config/${id}`, { headers });
    ...
    console.error("[BFF:riscos-config:GET:id]", error);
    return NextResponse.json({ message: "Falha ao buscar configuração de risco." }, { status: 500 });
  }
}
...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }): Promise<NextResponse> {
  try {
    ...
    const { headers } = await getAuthHeaders();
    const response = await fetch(`${NEST_URL}riscos-config/${id}`, {
      method: "PATCH",
      headers,
    ...
    console.error("[BFF:riscos-config:PATCH]", error);
    return NextResponse.json({ message: "Falha ao atualizar configuração de risco." }, { status: 500 });
  }
}
...
export async function DELETE(_req: Request, { params }: { params: Promise<{ id: string }> }): Promise<NextResponse> {
  try {
    ...
    console.error("[BFF:riscos-config:DELETE]", error);
    return NextResponse.json({ message: "Falha ao excluir configuração de risco." }, { status: 500 });
  }
}
