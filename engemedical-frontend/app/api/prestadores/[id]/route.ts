...
export async function GET(_req: Request, { params }: { params: Promise<{ id: string }> }): Promise<NextResponse> {
  try {
    ...
    const response = await fetch(`${NEST_URL}prestadores/${id}`, { headers });
    ...
    console.error("[BFF:prestadores:GET:id]", error);
    return NextResponse.json({ message: "Falha ao buscar prestador." }, { status: 500 });
  }
}
...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }): Promise<NextResponse> {
  try {
    ...
    const { headers } = await getAuthHeaders();
    const response = await fetch(`${NEST_URL}prestadores/${id}`, {
      method: "PATCH",
      headers,
    ...
    console.error("[BFF:prestadores:PATCH]", error);
    return NextResponse.json({ message: "Falha ao atualizar prestador." }, { status: 500 });
  }
}
...
export async function DELETE(_req: Request, { params }: { params: Promise<{ id: string }> }): Promise<NextResponse> {
  try {
    ...
    console.error("[BFF:prestadores:DELETE]", error);
    return NextResponse.json({ message: "Falha ao excluir prestador." }, { status: 500 });
  }
}
