...
export async function POST(req: Request) {
  try {
    ...
    const url = action === 'confirm' && form.get('id') ? `${NEST_URL}soc/history-import/${encodeURIComponent(String(form.get('id')))}/confirm` : `${NEST_URL}soc/history-import/analyze`;
    const body = action === 'confirm' ? JSON.stringify({ employeeId: form.get('employeeId'), documentIds: JSON.parse(String(form.get('documentIds') || '[]')) }) : (() => { const upload = new FormData(); const file = form.get('file'); if (file) upload.append('file', file); return upload; })();
