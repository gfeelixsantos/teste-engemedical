...
export async function GET(): Promise<NextResponse> {
  try {
    ...
    const response = await fetch(`${NEST_URL}orientacoes-config`, { headers });
    ...
    console.error("[BFF:orientacoes-config:GET]", error);
    return NextResponse.json(
      { message: "Falha ao listar orientações de parecer." },
      { status: 500 },
    );
  }
}
...
export async function POST(req: Request): Promise<NextResponse> {
  try {
    ...
    const { headers } = await getAuthHeaders();
    const response = await fetch(`${NEST_URL}orientacoes-config`, {
      method: "POST",
      headers,
    ...
    console.error("[BFF:orientacoes-config:POST]", error);
    return NextResponse.json(
      { message: "Falha ao criar orientação de parecer." },
      { status: 500 },
    );
  }
}
