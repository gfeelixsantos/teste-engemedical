...
export async function GET(
  ...
  try {
    ...
    console.error("[BFF:orientacoes-config:GET:id]", error);
    return NextResponse.json(
      { message: "Falha ao buscar orientação de parecer." },
      { status: 500 },
    );
  }
}
...
export async function PATCH(
  ...
  try {
    ...
    console.error("[BFF:orientacoes-config:PATCH]", error);
    return NextResponse.json(
      { message: "Falha ao atualizar orientação de parecer." },
      { status: 500 },
    );
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    console.error("[BFF:orientacoes-config:DELETE]", error);
    return NextResponse.json(
      { message: "Falha ao excluir orientação de parecer." },
      { status: 500 },
    );
  }
}
