...
function ArrayField({ label, value }: { label: string; value: unknown[] }) {
      ...
      <div className="flex flex-col">
        ...
        <div className="mt-1 flex flex-wrap gap-1">
          {value.slice(0, 10).map((item, i) => (
            <span
              key={i}
              className="inline-block rounded bg-[#28B1CF]/10 px-2 py-0.5 text-xs font-medium text-[#28B1CF]"
            >
              {formatValue(item)}
            </span>
          ))}
  ...
  return (
    <div className="flex flex-col">
      <button
        type="button"
        className="flex items-center gap-1 text-[10px] font-medium uppercase tracking-wider text-gray-400 hover:text-[#28B1CF]"
...
function JsonToggle({ payload }: { payload: Record<string, unknown> }) {
  ...
  return (
    <div className="mt-3">
      <button
        type="button"
        className="flex items-center gap-1.5 rounded-lg border border-gray-200 px-3 py-1.5 text-xs font-medium text-gray-600 transition-colors hover:border-[#28B1CF]/30 hover:bg-[#28B1CF]/5 hover:text-[#28B1CF]"
        onClick={() => setExpanded(!expanded)}
      >
      ...
      >
        ...
        <div className="p-5">
          ...
          <textarea
            ...
            className="h-64 w-full rounded-lg border border-gray-200 bg-gray-900 p-3 font-mono text-xs leading-relaxed text-green-400 focus:border-[#28B1CF] focus:outline-none focus:ring-1 focus:ring-[#28B1CF]"
            spellCheck={false}
          />
...
export function QueueDetailPanel({
  ...
  return (
    ...
    >
      <div className="rounded-xl border border-[#28B1CF]/20 bg-white shadow-lg">
        {/* Header */}
        <div className="flex items-center justify-between border-b border-[#28B1CF]/15 bg-gradient-to-r from-[#28B1CF] to-[#006782] px-5 py-4">
          ...
          {workerInfo && (
            <div className="mb-5 grid grid-cols-2 gap-3 sm:grid-cols-4">
              <div className="rounded-lg border border-gray-100 bg-gray-50 p-3">
                ...
                <p className="mt-1 text-xl font-bold text-[#28B1CF]">
                  {messageCount}
                </p>
              </div>
